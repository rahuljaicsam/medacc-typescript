...
const CloudComputingModule = () => {
  ...
  const module = cloudComputingCourseData[moduleIndex];
  const nextModuleId = moduleIndex + 1 < cloudComputingCourseData.length ? moduleIndex + 2 : null;
      ...
      try {
        ...
        if (!docSnap.exists()) {
          await setDoc(userRef, {
            'cloud-computing-life-sciences': [module.id]
          });
        } else {
          await updateDoc(userRef, {
            'cloud-computing-life-sciences': arrayUnion(module.id)
          });
        }
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <div className="container mx-auto px-4 py-8">
        <Button 
          variant="ghost" 
          onClick={() => navigate('/course/cloud-computing-life-sciences')}
          className="mb-6"
        >
                  ...
                  <div className="text-center py-8 space-y-6">
                    ...
                    <div className="flex gap-4 justify-center">
                      ...
                      {score / module.quiz.length >= 0.7 && nextModuleId && (
                        <Button onClick={() => navigate(`/course/cloud-computing-life-sciences/module/${nextModuleId}`)}>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {cloudComputingCourseData.map((m, i) => (
                    <div key={m.id} className="flex items-center gap-3 text-sm">
                      <div className={`
                        w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium
                        ${i === moduleIndex ? 'bg-primary text-primary-foreground' : 'bg-muted'}
                      `}>
                        {i + 1}
                      </div>
                      <span className={i === moduleIndex ? 'font-medium' : 'text-muted-foreground'}>
                        {m.title}
                      </span>
                      {i < moduleIndex && <CheckCircle2 className="w-4 h-4 text-green-600 ml-auto" />}
                    </div>
                  ))}
                </div>
              </CardContent>
