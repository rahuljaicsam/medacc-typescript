...
const RoboticsHealthcareModule = () => {
  ...
  const module = roboticsHealthcareCourseModules.find(m => m.id === Number(moduleId));
  ...
  const updateProgress = async (finalScore: number) => {
    ...
    try {
      ...
      const moduleData = {
        ...
        attempts: (docSnap.exists() && docSnap.data()['robotics-healthcare']?.modules?.[`module-${moduleId}`]?.attempts || 0) + 1,
      };

      if (docSnap.exists()) {
        await updateDoc(docRef, {
          [`robotics-healthcare.modules.module-${moduleId}`]: moduleData,
          'robotics-healthcare.lastUpdated': serverTimestamp()
        });
      } else {
        await setDoc(docRef, {
          'robotics-healthcare': {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-white border-b h-16 fixed w-full top-0 z-30 flex items-center justify-between px-4 lg:px-8">
        <div className="flex items-center gap-4">
          <button 
            onClick={() => navigate('/course/robotics-healthcare')}
        ...
        `}>
          <div className="p-4">
            ...
            <div className="space-y-1">
              {roboticsHealthcareCourseModules.map((m) => (
                <button
                  key={m.id}
                  onClick={() => {
                    navigate(`/course/robotics-healthcare/module/${m.id}`);
            ...
            <AnimatePresence mode="wait">
              {activeTab === 'content' ? (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: 20 }}
                  className="bg-white rounded-2xl p-8 shadow-sm border prose prose-orange max-w-none"
                        ...
                        {(quizScore / module.quiz.length) >= 0.7 && (
                          <button
                            onClick={() => {
                              ...
                              if (nextId <= roboticsHealthcareCourseModules.length) {
                                navigate(`/course/robotics-healthcare/module/${nextId}`);
                                setShowResults(false);
                                setCurrentQuizQuestion(0);
                                setSelectedAnswers({});
                            ...
                            className="px-6 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700"
                          >
                            {module.id === roboticsHealthcareCourseModules.length ? 'Finish Course' : 'Next Module'}
                          </button>
                        )}
                      </div>
                    </div>
...
export default RoboticsHealthcareModule;
