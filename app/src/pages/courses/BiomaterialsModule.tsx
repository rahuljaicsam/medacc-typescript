...
const BiomaterialsModule = () => {
  ...
  const module = biomaterialsCourseModules.find(m => m.id === Number(moduleId));
  ...
  const updateProgress = async (finalScore: number) => {
    ...
    try {
      ...
      const moduleData = {
        ...
        attempts: (docSnap.exists() && docSnap.data()['biomaterials-innovation']?.modules?.[`module-${moduleId}`]?.attempts || 0) + 1,
      };

      if (docSnap.exists()) {
        await updateDoc(docRef, {
          [`biomaterials-innovation.modules.module-${moduleId}`]: moduleData,
          'biomaterials-innovation.lastUpdated': serverTimestamp()
        });
      } else {
        await setDoc(docRef, {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-white border-b h-16 fixed w-full top-0 z-30 flex items-center justify-between px-4 lg:px-8">
        <div className="flex items-center gap-4">
          <button 
            onClick={() => navigate('/course/biomaterials')}
        ...
        `}>
          <div className="p-4">
            ...
            <div className="space-y-1">
              {biomaterialsCourseModules.map((m) => (
                <button
                  key={m.id}
                  onClick={() => {
                    navigate(`/course/biomaterials/module/${m.id}`);
            ...
            <AnimatePresence mode="wait">
              {activeTab === 'content' ? (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: 20 }}
                  className="bg-white rounded-2xl p-8 shadow-sm border prose prose-purple max-w-none"
                        ...
                        {(quizScore / module.quiz.length) >= 0.7 && (
                          <button
                            onClick={() => {
                              ...
                              if (nextId <= biomaterialsCourseModules.length) {
                                navigate(`/course/biomaterials/module/${nextId}`);
                                setShowResults(false);
                                setCurrentQuizQuestion(0);
                                setSelectedAnswers({});
                            ...
                            className="px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700"
                          >
                            {module.id === biomaterialsCourseModules.length ? 'Finish Course' : 'Next Module'}
                          </button>
                        )}
                      </div>
                    </div>
...
export default BiomaterialsModule;
