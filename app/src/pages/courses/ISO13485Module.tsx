...
const ISO13485Module = () => {
  ...
  const module = iso13485CourseModules.find(m => m.id === Number(moduleId));
  ...
  const checkProgress = async (uid: string) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        const moduleStatus = data['iso-13485-qms']?.modules?.[`module-${moduleId}`]?.status;
  ...
  const updateProgress = async (finalScore: number) => {
    ...
    try {
      ...
      const moduleData = {
        ...
        attempts: (docSnap.exists() && docSnap.data()['iso-13485-qms']?.modules?.[`module-${moduleId}`]?.attempts || 0) + 1,
      };

      if (docSnap.exists()) {
        await updateDoc(docRef, {
          [`iso-13485-qms.modules.module-${moduleId}`]: moduleData,
          'iso-13485-qms.lastUpdated': serverTimestamp()
        });
      } else {
        await setDoc(docRef, {
          'iso-13485-qms': {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => navigate('/course/iso13485')}>Back to Course</Button>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <Button 
              ...
              onClick={() => navigate('/course/iso13485')}
            >
                    ...
                    {!showScore ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <div className="flex justify-center gap-4">
                          ...
                          <Button onClick={() => navigate('/course/iso13485')}>
                            Back to Course
                          </Button>
                        </div>
                      </div>
