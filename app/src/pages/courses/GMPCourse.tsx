...
import { gmpCourseModules } from '@/data/gmpCourseData';
  ...
  const fetchProgress = async (uid: string) => {
    try {
      ...
      if (docSnap.exists()) {
        ...
        const courseData = data['gmp-fundamentals'];
        ...
        if (courseData && courseData.modules) {
          ...
          setProgress(Math.round((completed.length / gmpCourseModules.length) * 100));
        }
      }
    } catch (error) {
      ...
      <main className="pt-24 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
              <div>
                <Badge className="mb-2 bg-blue-100 text-blue-700 hover:bg-blue-100 border-none">
                  GMP Manufacturing Fundamentals
                </Badge>
                <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
                  Good Manufacturing Practice Mastery
                </h1>
                <p className="text-lg text-gray-600 max-w-3xl">
                  Master the essential quality standards and regulations required for scaling biotech and pharmaceutical production.
                </p>
              </div>
              ...
              <Card className="p-6 bg-white shadow-sm border-gray-100 min-w-[300px]">
                ...
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  ...
                  <span>{modulesCompletedCount} of {gmpCourseModules.length} modules completed</span>
                </div>
              </Card>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {gmpCourseModules.map((module, index) => {
              const isCompleted = completedModules.includes(module.id);
              const isLocked = index > 0 && !completedModules.includes(gmpCourseModules[index - 1].id) && !isCompleted;
              ...
              return (
                ...
                >
                  <Card 
                    ...
                    onClick={() => !isLocked && navigate(`/course/gmp/module/${module.id}`)}
                  >
              ...
              <Card className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-8 md:p-12 text-center rounded-3xl overflow-hidden relative">
                <div className="relative z-10">
                  ...
                  <p className="text-blue-100 text-lg max-w-2xl mx-auto mb-8">
                    You've completed the GMP Manufacturing Fundamentals course. You are now equipped with the knowledge to navigate quality manufacturing environments.
                  </p>
                  <Button 
                    size="lg" 
