...
const MedicalModule = () => {
  ...
  const module = medicalCourseModules.find(m => m.id === Number(moduleId));
  ...
  const checkProgress = async (uid: string) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        const moduleStatus = data['medical-essentials']?.modules?.[`module-${moduleId}`]?.status;
  ...
  const updateProgress = async (finalScore: number) => {
    ...
    try {
      ...
      const moduleData = {
        ...
        attempts: (docSnap.exists() && docSnap.data()['medical-essentials']?.modules?.[`module-${moduleId}`]?.attempts || 0) + 1,
      };

      if (docSnap.exists()) {
        await updateDoc(docRef, {
          [`medical-essentials.modules.module-${moduleId}`]: moduleData,
          'medical-essentials.lastUpdated': serverTimestamp()
        });
      } else {
        await setDoc(docRef, {
          'medical-essentials': {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => navigate('/course/medical')}>Back to Course</Button>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <Button 
              ...
              onClick={() => navigate('/course/medical')}
            >
                    ...
                    {!showScore ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <div className="flex justify-center gap-4">
                          ...
                          <Button onClick={() => navigate('/course/medical')}>
                            Back to Course
                          </Button>
                        </div>
                      </div>
