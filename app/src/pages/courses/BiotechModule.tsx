...
const BiotechModule = () => {
  ...
  const module = biotechCourseModules.find(m => m.id === Number(moduleId));
  ...
  const checkProgress = async (uid: string) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        const moduleStatus = data['biotech-mastery']?.modules?.[`module-${moduleId}`]?.status;
  ...
  const updateProgress = async (finalScore: number) => {
    ...
    try {
      ...
      const moduleData = {
        ...
        attempts: (docSnap.exists() && docSnap.data()['biotech-mastery']?.modules?.[`module-${moduleId}`]?.attempts || 0) + 1,
      };

      if (docSnap.exists()) {
        await updateDoc(docRef, {
          [`biotech-mastery.modules.module-${moduleId}`]: moduleData,
          'biotech-mastery.lastUpdated': serverTimestamp()
        });
      } else {
        await setDoc(docRef, {
          'biotech-mastery': {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => navigate('/course/biotech')}>Back to Course</Button>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <Button 
              ...
              onClick={() => navigate('/course/biotech')}
            >
                    ...
                    {!showScore ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <p className="text-gray-600 mb-6">
                          You scored {score} out of {module.quiz.length}
                        </p>
                        ...
                        <div className="flex justify-center gap-4">
                          ...
                          <Button onClick={() => navigate('/course/biotech')}>
                            Back to Course
                          </Button>
                        </div>
                      </div>
