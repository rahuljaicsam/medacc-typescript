...
const DigitalHealthDataPrivacyModule = () => {
  ...
  const module = digitalHealthDataPrivacyCourseModules.find(m => m.id === Number(moduleId));
  ...
  const updateProgress = async (finalScore: number) => {
    ...
    try {
      ...
      const moduleData = {
        ...
        attempts: (docSnap.exists() && docSnap.data()['digital-health-data-privacy']?.modules?.[`module-${moduleId}`]?.attempts || 0) + 1,
      };

      if (docSnap.exists()) {
        await updateDoc(docRef, {
          [`digital-health-data-privacy.modules.module-${moduleId}`]: moduleData,
          'digital-health-data-privacy.lastUpdated': serverTimestamp()
        });
      } else {
        await setDoc(docRef, {
      ...
      <header className="bg-white border-b sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center">
            <button 
              onClick={() => navigate('/course/digital-health-data-privacy')}
        ...
        `}>
          <div className="p-4 overflow-y-auto h-full">
            ...
            <div className="space-y-2">
              {digitalHealthDataPrivacyCourseModules.map((m) => (
                <button
                  key={m.id}
                  onClick={() => {
                    navigate(`/course/digital-health-data-privacy/module/${m.id}`);
                ...
                >
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center mr-3 text-xs ${
                    m.id === module.id ? 'bg-emerald-200' : 'bg-gray-200'
                  }`}>
                    {m.id}
                  </div>
                        ...
                        {(quizScore / module.quiz.length) >= 0.7 && (
                          <button
                            onClick={() => {
                              ...
                              if (nextId <= digitalHealthDataPrivacyCourseModules.length) {
                                navigate(`/course/digital-health-data-privacy/module/${nextId}`);
                                setShowResults(false);
                                setCurrentQuizQuestion(0);
                                setSelectedAnswers({});
                          ...
                          >
                            {module.id === digitalHealthDataPrivacyCourseModules.length ? 'Finish Course' : 'Next Module'}
                          </button>
                        )}
                      </div>
                    </div>
...
export default DigitalHealthDataPrivacyModule;
