...
const AppDevelopmentHealthcareModule = () => {
  ...
  const module = appDevelopmentHealthcareCourseModules.find(m => m.id === Number(moduleId));
  ...
  const updateProgress = async (finalScore: number) => {
    ...
    try {
      ...
      const moduleData = {
        ...
        attempts: (docSnap.exists() && docSnap.data()['app-development-healthcare']?.modules?.[`module-${moduleId}`]?.attempts || 0) + 1,
      };

      if (docSnap.exists()) {
        await updateDoc(docRef, {
          [`app-development-healthcare.modules.module-${moduleId}`]: moduleData,
          'app-development-healthcare.lastUpdated': serverTimestamp()
        });
      } else {
        await setDoc(docRef, {
      ...
      <header className="bg-white border-b sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center">
            <button 
              onClick={() => navigate('/course/app-development-healthcare')}
        ...
        `}>
          <div className="p-4 overflow-y-auto h-full">
            ...
            <div className="space-y-2">
              {appDevelopmentHealthcareCourseModules.map((m) => (
                <button
                  key={m.id}
                  onClick={() => {
                    navigate(`/course/app-development-healthcare/module/${m.id}`);
                        ...
                        {(quizScore / module.quiz.length) >= 0.7 && (
                          <button
                            onClick={() => {
                              ...
                              if (nextId <= appDevelopmentHealthcareCourseModules.length) {
                                navigate(`/course/app-development-healthcare/module/${nextId}`);
                                setShowResults(false);
                                setCurrentQuizQuestion(0);
                                setSelectedAnswers({});
                          ...
                          >
                            {module.id === appDevelopmentHealthcareCourseModules.length ? 'Finish Course' : 'Next Module'}
                          </button>
                        )}
                      </div>
                    </div>
...
export default AppDevelopmentHealthcareModule;
