...
import { techGeneralKnowledgeCourseModules } from '../../data/techGeneralKnowledgeCourseData';
import { auth, db } from '../../lib/firebase';
import { doc, getDoc, updateDoc, setDoc, serverTimestamp } from 'firebase/firestore';
import { toast } from 'sonner';
import confetti from 'canvas-confetti';

const TechGeneralKnowledgeModule = () => {
  ...
  const module = techGeneralKnowledgeCourseModules.find(m => m.id === Number(moduleId));
  ...
  const updateProgress = async (finalScore: number) => {
    ...
    try {
      ...
      const moduleData = {
        ...
        attempts: (docSnap.exists() && docSnap.data()['tech-general-knowledge']?.modules?.[`module-${moduleId}`]?.attempts || 0) + 1,
      };

      if (docSnap.exists()) {
        await updateDoc(docRef, {
          [`tech-general-knowledge.modules.module-${moduleId}`]: moduleData,
          'tech-general-knowledge.lastUpdated': serverTimestamp()
        });
      } else {
        await setDoc(docRef, {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-white border-b h-16 fixed w-full top-0 z-30 flex items-center justify-between px-4 lg:px-8">
        <div className="flex items-center gap-4">
          <button 
            onClick={() => navigate('/course/tech-general-knowledge')}
        ...
        `}>
          <div className="p-4">
            ...
            <div className="space-y-1">
              {techGeneralKnowledgeCourseModules.map((m) => (
                <button
                  key={m.id}
                  onClick={() => {
                    navigate(`/course/tech-general-knowledge/module/${m.id}`);
            ...
            <AnimatePresence mode="wait">
              {activeTab === 'content' ? (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: 20 }}
                  className="bg-white rounded-2xl p-8 shadow-sm border prose prose-blue max-w-none"
                        ...
                        {(quizScore / module.quiz.length) >= 0.7 && (
                          <button
                            onClick={() => {
                              ...
                              if (nextId <= techGeneralKnowledgeCourseModules.length) {
                                navigate(`/course/tech-general-knowledge/module/${nextId}`);
                                setShowResults(false);
                                setCurrentQuizQuestion(0);
                                setSelectedAnswers({});
                            ...
                            className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
                          >
                            {module.id === techGeneralKnowledgeCourseModules.length ? 'Finish Course' : 'Next Module'}
                          </button>
                        )}
                      </div>
                    </div>
...
export default TechGeneralKnowledgeModule;
