...
const GMPModule = () => {
  ...
  const module = gmpCourseModules.find(m => m.id === Number(moduleId));
  ...
  const checkProgress = async (uid: string) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        const moduleStatus = data['gmp-fundamentals']?.modules?.[`module-${moduleId}`]?.status;
  ...
  const updateProgress = async (finalScore: number) => {
    ...
    try {
      ...
      const moduleData = {
        ...
        attempts: (docSnap.exists() && docSnap.data()['gmp-fundamentals']?.modules?.[`module-${moduleId}`]?.attempts || 0) + 1,
      };

      if (docSnap.exists()) {
        await updateDoc(docRef, {
          [`gmp-fundamentals.modules.module-${moduleId}`]: moduleData,
          'gmp-fundamentals.lastUpdated': serverTimestamp()
        });
      } else {
        await setDoc(docRef, {
          'gmp-fundamentals': {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => navigate('/course/gmp')}>Back to Course</Button>
        </div>
      </div>
    );
  }
                    ...
                    {!showScore ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <div className="flex justify-center gap-4">
                          ...
                          <Button onClick={() => navigate('/course/gmp')}>
                            Back to Course
                          </Button>
                        </div>
                      </div>
